...
class nnUNetPredictor(object):
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor, num_steps: int) -> torch.Tensor:
        mirror_axes = self.allowed_mirroring_axes if self.use_mirroring else None
        start_time = time.time()
        prediction = self.network(x)
        end_time = time.time()
        execution_time = end_time - start_time
        total_time = num_steps*execution_time
        
        allowed_time_per_fold = 30
        #print(execution_time)
        #print(total_time)

        if total_time > 0:
            limit_axe = min(int((allowed_time_per_fold-total_time)/total_time), 7)
        else:
            limit_axe = 0
        ...
        if mirror_axes is not None:
            ...
            for axes in axes_combinations[:limit_axe]:
                prediction += torch.flip(self.network(torch.flip(x, axes)), axes)
                
            prediction /= (limit_axe + 1)
        return prediction
    ...
    def _internal_predict_sliding_window_return_logits(self,
        ...
        try:
            ...
            for sl in tqdm(slicers, disable=not self.allow_tqdm):
                ...
                #start_time = time.time()
                prediction = self._internal_maybe_mirror_and_predict(workon, num_steps=len(slicers))[0].to(results_device)
                #end_time = time.time()
                #execution_time = end_time - start_time
                #print(execution_time)
                if self.use_gaussian:
